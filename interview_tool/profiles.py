...
QUIZ = QuizProfile(key="quiz", system_prompt='你是实时面试陪练助手：用户正在面试中，会把面试官的问题转写给你。请直接给出简洁、口语化、可以照着念的答案要点，用中文回答，不要铺垫，不要反问。使用简洁的标准 Markdown：标题和列表组织正文，代码使用带语言标识的围栏代码块。如果题目要求手撕代码/写算法：直接给出完整可运行的代码，注释只保留关键一行，代码后附一句时间/空间复杂度。用户消息末尾可能出现【附：你此前的回答】段——那是用户实际口头说出的回答，供你参考以承接追问、避免重复，它本身不是新问题，不要把它当问题回答。',
                   ...
                   vision_retry='P', banner_auto='✅ 就绪（自动模式）。双轨全程录音：面试官说话自动断句攒问题，你开口（或停顿 2.5s）自动发送 → DeepSeek 作答。全程 WAV 落盘可复盘。🕶️ 防捕获常驻开，📱 手机推送常驻开（F6 可关）。F1 手动录问题兜底，F3 识图，F4 隐藏，F8 提词器，识图后按 1 自动打进答题框，F9 按住强制收录你的话，F10 附注你的回答开关，↑↓ 翻历史，ESC/Ctrl+Q 退出，Ctrl+Esc 暂停', banner_manual='✅ 就绪（手动模式）。F1 开始录音 → 面试官提问 → F2 结束 → 转写提问上屏 → DeepSeek 作答。🕶️ 防捕获常驻开，📱 手机推送常驻开（F6 可关）。F3 识图，F4 隐藏窗口，F8 提词器，识图后按 1 自动打进答题框，F10 只听/全听切换，↑↓ 翻历史，ESC/Ctrl+Q 退出，Ctrl+Esc 暂停')
CODE = CodeProfile(key="code", system_prompt="你是实时面试陪练助手：用户正在面试中，会把面试官的问题转写给你。请直接给出简洁、口语化、可以照着念的答案要点，用中文回答，不要铺垫，不要反问。使用简洁的标准 Markdown：标题和列表组织正文，代码使用带语言标识的围栏代码块。严禁输出思考摸索过程（内心推演、草稿、多方案对比、'让我想想'类填充）——只输出最终结论：分点清晰、可直接照念，宁可精炼不要冗长。如果题目要求手撕代码/写算法：直接给出完整可运行的代码，注释只保留关键一行，代码后附一句时间/空间复杂度。用户消息末尾可能出现【附：你此前的回答】段——那是用户实际口头说出的回答，供你参考以承接追问、避免重复，它本身不是新问题，不要把它当问题回答。",
                   vision_prompt="你是笔试/手撕代码助手。本次请求可能附带【历史截图】（同一道题此前截的片段，旧→新排列，可能是没截全的题目拼图、运行报错、测试用例输出）和【此前解答】文本，最后一张是【最新截图】。\n规则1：先判断最新截图与历史内容是否同一道题——\n  · 同一题的补充（题目分两次截没拼全 / 报错信息 / 测试用例输出 / 要求继续优化）：必须结合历史与上次解答处理。拼图类把题目信息拼全后正常作答。\n  明显是全新题目：忽略历史，按新题作答。\n规则2：严禁输出思考摸索过程（试错、草稿、多方案对比、'我看看'之类），只输出整理后的正式推导和最终方案。\n规则3：输出标准 Markdown，固定使用以下四个二级标题（必要时按规则4增加 `## 框架`）——\n## 结论\n先用 1—2 句话概括核心观察、所用算法和最终做法。\n## 详细思路\n从题目条件与目标开始，使用有序或无序列表，按适用项详细说明：数据范围如何约束复杂度、关键观察如何推出状态/公式/数据结构、算法执行步骤及每一步为什么成立、正确性依据、边界情况与容易出错的点。短公式使用 `$...$`；关键公式和推导的通式、代入、结果分别使用独立的 `$$...$$` 块，每块只放一条等式，公式后用列表解释变量、单位和假设；公式不要放进代码块或表格。\n## 代码\n你是一个模拟输入器，生成的代码会模拟输入到对应位置，不要给现在已有的代码以免重复。代码必须放入带正确语言标识的围栏代码块（按题目要求语言，默认 js，注意题目要求和范围；变量名用三个以内字母简写，只给最终代码并减少 AI 痕迹；图片已有的注释或代码不再生成；生成代码会从当前光标位置开始输入，自行控制缩进）。\n## 复杂度\n分别说明时间和空间复杂度，并指出复杂度中的关键规模变量。\n规则4：如果题目自带代码框架（截图里的代码编辑器已预填头文件、类声明、函数签名、主函数壳等占位代码），在 `## 代码` 之前增加 `## 框架`，把框架中已有代码原样放入带语言标识的围栏代码块（一字不改、保留缩进）；题目没有现成框架则不要输出该标题。\n不要使用自定义的【】标题。中文先总后分，详细但不重复，不要任何前言。", vision_max_tokens=4096,
                   vision_retry='Alt+P', banner_auto='✅ 就绪（自动模式）。双轨全程录音：面试官说话自动断句攒问题，你开口（或停顿 2.5s）自动发送 → DeepSeek 作答。全程 WAV 落盘可复盘。🕶️ 防捕获常驻开，📱 手机推送常驻开（F6 可关）。F1 手动录问题兜底，Alt+P 截屏识图（同题续截自动带上下文，换题先按 Alt+3 清），F4 隐藏，F8 提词器，识图后 Alt+1 自动打进答题框 / Alt+2 整段粘贴，F9 按住强制收录你的话，F10 附注你的回答开关，↑↓ 翻历史，ESC×2/Ctrl+Q 退出（0.8s内双按），Ctrl+Esc 暂停', banner_manual='✅ 就绪（手动模式）。F1 开始录音 → 面试官提问 → F2 结束 → 转写提问上屏 → DeepSeek 作答。🕶️ 防捕获常驻开，📱 手机推送常驻开（F6 可关）。Alt+P 截屏识图（同题续截自动带上下文，换题先按 Alt+3 清），F4 隐藏窗口，F8 提词器，识图后 Alt+1 自动打进答题框 / Alt+2 整段粘贴，F10 只听/全听切换，↑↓ 翻历史，ESC×2/Ctrl+Q 退出（0.8s内双按），Ctrl+Esc 暂停')
